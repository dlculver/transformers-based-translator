...
class TranslationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        src_tokens = torch.tensor(
            self.pad_and_truncate(self.add_special_tokens(src_tokens))
        )
        tgt_tokens = torch.tensor(
            self.pad_and_truncate(self.add_special_tokens(tgt_tokens))
        )
...
def collate_fn(batch, pad_token_id: int):
    ...
    # create masks
    src_mask = (
        (src_batch != pad_token_id).unsqueeze(-2).int()
    )  # shape: (bs, 1, seq_length)
    tgt = tgt_batch[:, :-1]
    tgt_y = tgt_batch[:, 1:]
